...
assert.match(subscribe, /billingCycle === 'monthly'/);
assert.match(subscribe, /createCheckoutSession\(window\.location\.origin, \{ billingCycle, attribution \}\)/);
assert.match(subscribe, /verifyPayPalSubscription\(data\.subscriptionID, billingCycle/);
assert.match(subscribe, /ANNUAL_SAVINGS_PERCENT/);
assert.match(subscribe, /custom_id: customId/);
