...
export default function IconRenderer({ el, scale, x, y, selected, onDragStart, onDragEnd, onDragMove, onClick, onTap }) {
  ...
  const selRing = (r) => selected
    ...
    : null;
  const deleteIndicator = ({ x = -half - 8, y = -half - 8, width = size + 16, height = size + 16, cornerRadius = 6 } = {}) => (
    <Rect
      name="delIndicator"
      x={x}
      y={y}
  ...
  if (el.type === 'goal-large') {
    ...
    return (
      <Group {...groupProps}>
        {deleteIndicator({ x: -w / 2 - 8, y: -h / 2 - 8, width: w + 16, height: h + 16 })}
  ...
  if (el.type === 'ladder') {
    ...
    return (
      <Group {...groupProps}>
        {deleteIndicator({ x: x0 - 8, y: y0 - 8, width: w + 16, height: h + 16 })}
