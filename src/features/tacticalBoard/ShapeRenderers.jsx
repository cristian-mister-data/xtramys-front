  ...
  return {
    ...
    onDragMove: (e) => {
      ...
      const checkPoints = el.points
        ? el.points.map((p) => { const px = r2p(p.x, p.y); return { x: px.x + dx, y: px.y + dy }; })
        : null;
      const checkXY = (typeof el.x === 'number' && typeof el.y === 'number' && !checkPoints)
        ? (() => { const px = r2p(el.x, el.y); return { x: px.x + dx, y: px.y + dy }; })()
        : null;
      let outside = false;
      if (checkPoints) { outside = checkPoints.some((pt) => { const r = p2r(pt.x, pt.y); return isOutsideVisibleField(r.x, r.y, viewMode, size.w, size.h); }); }
      else if (checkXY) { const r = p2r(checkXY.x, checkXY.y); outside = isOutsideVisibleField(r.x, r.y, viewMode, size.w, size.h); }
      const ring = node.findOne('.delIndicator');
      if (outside) { node.opacity(0.4); node.scaleX(0.75); node.scaleY(0.75); if (ring) ring.visible(true); }
      else { node.opacity(1); node.scaleX(1); node.scaleY(1); if (ring) ring.visible(false); }
    ...
    onDragEnd: (e) => {
      ...
      const ring = node.findOne('.delIndicator');
      if (ring) ring.visible(false);
      if (dx === 0 && dy === 0) { node.getLayer()?.batchDraw(); return; }
      node.position({ x: 0, y: 0 });
      const newPoints = el.points ? el.points.map((p) => { const px = r2p(p.x, p.y); return p2r(px.x + dx, px.y + dy); }) : null;
      const newXY = (typeof el.x === 'number' && typeof el.y === 'number' && !newPoints) ? (() => { const px = r2p(el.x, el.y); return p2r(px.x + dx, px.y + dy); })() : null;
      let outside = false;
      if (newPoints) { outside = newPoints.some((pt) => isOutsideVisibleField(pt.x, pt.y, viewMode, size.w, size.h)); }
      else if (newXY) { outside = isOutsideVisibleField(newXY.x, newXY.y, viewMode, size.w, size.h); }
      if (outside) { applyChange((prev) => prev.filter((it) => it.id !== el.id)); }
      else { applyChange((prev) => prev.map((it) => { if (it.id !== el.id) return it; if (newPoints) return { ...it, points: newPoints }; if (newXY) return { ...it, x: newXY.x, y: newXY.y }; return it; })); }
      node.getLayer()?.batchDraw();
    },
  };
}
...
function VertexHandle({ x, y, onUpdate }) {
  return <Circle x={x} y={y} radius={HANDLE_RADIUS} fill={HANDLE_FILL} stroke={HANDLE_STROKE} strokeWidth={1.5} draggable onDragEnd={(e) => { e.cancelBubble = true; onUpdate(e.target.x(), e.target.y()); }} onMouseDown={(e) => { e.cancelBubble = true; }} onClick={(e) => { e.cancelBubble = true; }} />;
}

export function LineShape({ el, selected, onSelect, p2r, r2p, applyChange, refScale, viewMode, size }) {
  const points = el.points.flatMap((p) => { const px = r2p(p.x, p.y); return [px.x, px.y]; });
  ...
  const updateVertex = (idx) => (px, py) => { const r = p2r(px, py); applyChange((prev) => prev.map((it) => { if (it.id !== el.id) return it; const nextPts = it.points.map((p, i) => (i === idx ? { x: r.x, y: r.y } : p)); return { ...it, points: nextPts }; })); };
  ...
  return (
    <Group {...bodyHandlers} onClick={onSelect} onTap={onSelect}>
      <Rect name="delIndicator" x={minX - 8} y={minY - 8} width={(maxX - minX || 20) + 16} height={(maxY - minY || 20) + 16} stroke="#ff0000" strokeWidth={3} dash={[6, 4]} fill="rgba(255,0,0,0.2)" cornerRadius={4} listening={false} visible={false} />
      <Comp points={points} stroke={selected ? '#fbbf24' : stroke} strokeWidth={selected ? strokeWidth + 2 : strokeWidth} tension={tension} dash={dash} pointerLength={el.arrow ? strokeWidth * 3 : undefined} pointerWidth={el.arrow ? strokeWidth * 3 : undefined} fill={el.arrow ? stroke : undefined} hitStrokeWidth={Math.max(12, strokeWidth + 12)} />
      {selected && el.points.map((p, idx) => { const px = r2p(p.x, p.y); return <VertexHandle key={idx} x={px.x} y={px.y} onUpdate={updateVertex(idx)} />; })}
    </Group>
  );
}

export function RectShape({ el, selected, onSelect, p2r, r2p, applyChange, refScale, viewMode, size }) {
  const c1 = r2p(el.x, el.y); const c2 = r2p(el.x + el.w, el.y + el.h);
...
export function CircleShape({ el, selected, onSelect, p2r, r2p, applyChange, refScale, viewMode, size }) {
