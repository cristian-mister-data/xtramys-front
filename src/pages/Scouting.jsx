...
const TAGS = [
  'Rapido', 'Tecnico', 'Inteligente', 'Competitivo', 'Trabajador',
  'Creativo', 'Fuerte', 'Buen pase', 'Buen disparo', 'Buen 1 vs 1',
  'Lider', 'Polivalente', 'Agresivo', 'Tranquilo con balon', 'Buena lectura tactica',
];
...
const emptyScores = () => SCORE_GROUPS.reduce((acc, [group, , fields]) => {
...
function normalizeForForm(report, team) {
  return {
    ...
    observationDate: report.observationDate ? new Date(report.observationDate).toISOString().slice(0, 10) : today(),
    age: report.age ?? '',
...
function cleanPayload(form) {
  ...
  SCORE_GROUPS.forEach(([group, , fields]) => {
    payload[group] = fields.reduce((acc, field) => ({ ...acc, [field]: cleanNumber(form[group]?.[field]) }), {});
  });
  return payload;
}
...
export default function Scouting() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return reports;
    return reports.filter((report) => [report.playerName, report.playerTeam, report.position, report.rival]
  ...
  const updateScore = (group, field, value) => setForm((prev) => ({
    ...prev,
  ...
  const handleDelete = async (report) => {
    const ok = await confirmAction(`Eliminar el informe de ${report.playerName}?`, { destructive: true });
    if (!ok) return;
    try {
  ...
  return (
    <Container>
      ...
      <Toolbar>
        <SearchBox>
          <MdSearch size={18} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar jugador, equipo, posicion..." />
        </SearchBox>
      ...
      <Grid>
        {filtered.map((report) => (
          <ReportCard key={report._id} onClick={() => setSelectedReport(report)} style={{ cursor: 'pointer' }}>
            <Row style={{ justifyContent: 'space-between', alignItems: 'center' }} $gap={8}>
              <CardTitle>{report.playerName}</CardTitle>
              {report.rating ? <Badge $tone="warning"><MdStar /> {report.rating}/10</Badge> : null}
            </Row>
            <CardMeta>
              {[report.position, report.playerTeam].filter(Boolean).join(' - ') || 'Sin equipo/posición'}
            </CardMeta>
            <TagList style={{ marginTop: 8 }}>
              {(report.tags || []).slice(0, 5).map((tag) => <Badge key={tag}>{tag}</Badge>)}
            </TagList>
          </ReportCard>
        ))}
      </Grid>
      ...
      >
        <form id="scouting-form" onSubmit={handleSave}>
          <Stack $gap={18}>
            <FormGrid>
              <Field><Label>Nombre</Label><Input value={form.playerName} onChange={(e) => update({ playerName: e.target.value })} required /></Field>
              <Field><Label>Edad</Label><Input type="number" min="0" max="80" value={form.age} onChange={(e) => update({ age: e.target.value })} /></Field>
              <Field><Label>Equipo</Label><Input value={form.playerTeam} onChange={(e) => update({ playerTeam: e.target.value })} /></Field>
              <Field>
              ...
              <Field>
                ...
                <Select value={form.dominantFoot} onChange={(e) => update({ dominantFoot: e.target.value })}>
              ...
              <Field><Label>Fecha observacion</Label><Input type="date" value={form.observationDate} onChange={(e) => update({ observationDate: e.target.value })} /></Field>
              <Field><Label>Rival</Label><Input value={form.rival} onChange={(e) => update({ rival: e.target.value })} /></Field>
              <Field><Label>Competicion</Label><Input value={form.competition} onChange={(e) => update({ competition: e.target.value })} /></Field>
              <Field><Label>Valoracion final</Label><Input type="number" min="0" max="10" value={form.rating} onChange={(e) => update({ rating: e.target.value })} /></Field>
            </FormGrid>
                ...
                <FormGrid>
                  {fields.map((field) => (
                    <Field key={field}>
                      ...
                      <Input type="number" min="0" max="10" value={form[group]?.[field] ?? ''} onChange={(e) => updateScore(group, field, e.target.value)} />
                    </Field>
                  ))}
                </FormGrid>
              </Stack>
            ...
            <Stack $gap={8}>
              ...
              <TagList>
                {TAGS.map((tag) => (
                  <TagButton key={tag} type="button" $active={form.tags.includes(tag)} onClick={() => toggleTag(tag)}>
                    {tag}
                  </TagButton>
                ))}
              </TagList>
            ...
            <TwoGrid>
              <Field>
                ...
                <Select value={form.recommendation} onChange={(e) => update({ recommendation: e.target.value })}>
              ...
              <Field>
                ...
                <Select value={form.potential} onChange={(e) => update({ potential: e.target.value })}>
            ...
            <TwoGrid>
              <Field><Label>Fortalezas</Label><TextArea value={form.strengths} onChange={(e) => update({ strengths: e.target.value })} /></Field>
              <Field><Label>Aspectos a mejorar</Label><TextArea value={form.improvements} onChange={(e) => update({ improvements: e.target.value })} /></Field>
            </TwoGrid>
            <Field><Label>Observaciones</Label><TextArea value={form.observations} onChange={(e) => update({ observations: e.target.value })} /></Field>
          </Stack>
        </form>
      </Modal>
      <ScoutingDetailModal
