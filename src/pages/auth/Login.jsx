import { useState, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { useTranslation } from 'react-i18next';
import { FaApple } from 'react-icons/fa';
import { loginThunk } from '@/store/slices/user/userThunks';
import { getGoogleOAuthURL } from '@/api/auth';
import { isNative, platform } from '@/platform/capacitor';
import { signInWithApple, signInWithAppleWeb } from '@/platform/appleSignIn';
import { openExternalWeb, websiteUrl } from '@/platform/externalWeb';
import {
  AccentLink,
  AuthFormShell,
  Divider,
  ErrorMessage,
  Field,
  Form,
  FormSubtitle,
  FormTitle,
  InputLabel,
  MutedAction,
  PrimaryButton,
  SecondaryLink,
  TextInput,
  normalizeEmail,
  SocialButton,
} from './AuthFormLayout';

export default function Login() {
  const { t, i18n } = useTranslation();
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const location = useLocation();
  const { loading, error, isAuthenticated, authChecked, user } = useSelector((state) => state.usuario);

  const [correo, setCorreo] = useState('');
  const [password, setPassword] = useState('');
  const [localError, setLocalError] = useState(null);
  const [appleLoading, setAppleLoading] = useState(false);

  useEffect(() => {
    if (authChecked && isAuthenticated) {
      const isClubAdmin = user?.role === 'club_admin' && user?.clubRole !== 'coach';
      navigate(isClubAdmin ? '/club/dashboard' : '/app', { replace: true });
    }
  }, [authChecked, isAuthenticated, navigate, user?.clubRole, user?.role]);

  const onSubmit = async (event) => {
    event.preventDefault();
    setLocalError(null);

    if (!correo || !password) {
      setLocalError(t('message.fieldRequired', 'Por favor completa este campo'));
      return;
    }

    const correoNorm = normalizeEmail(correo);
    try {
      const loggedUser = await dispatch(loginThunk({ correo: correoNorm, contraseña: password })).unwrap();
      const fromPath = location.state?.from?.pathname || '/app';
      const fromSearch = location.state?.from?.search || '';
      const from = fromPath + fromSearch;
      const isClubAdmin = loggedUser?.role === 'club_admin' && loggedUser?.clubRole !== 'coach';
      navigate(isClubAdmin && (!location.state?.from || fromPath === '/app') ? '/club/dashboard' : from, { replace: true });
    } catch (err) {
      const code = err?.code || err?.data?.code;
      if (code === 'EMAIL_NOT_VERIFIED') {
        navigate('/auth/verify-email', {
          state: { correo: err?.data?.correo || correoNorm, from: location.state?.from },
        });
        return;
      }
      setLocalError(err?.message || t('login.loginError', 'Correo o contraseña incorrectos'));
    }
  };

  const handleGoogleLogin = () => {
    const lang = i18n.language?.startsWith('es') ? 'es' : 'en';
    const fromPath = location.state?.from?.pathname || '/app';
    const params = new URLSearchParams(location.state?.from?.search || '');
    if (fromPath.startsWith('/subscribe') || fromPath.startsWith('/suscripcion')) {
      params.set('checkout', '1');
    }
    const from = fromPath + (params.toString() ? `?${params}` : '');
    window.location.href = getGoogleOAuthURL(lang, from);
  };

  const handleAppleLogin = async () => {
    setLocalError(null);
    setAppleLoading(true);
    try {
      const credential = platform === 'ios'
        ? await signInWithApple()
        : await signInWithAppleWeb();
      await dispatch(loginThunk({
        provider: 'apple',
        credential: {
          identityToken: credential.identityToken,
          authorizationCode: credential.authorizationCode,
          nonce: credential.nonce,
          nombre: credential.givenName?.trim().slice(0, 100),
          apellido: credential.familyName?.trim().slice(0, 100),
          idioma: i18n.language?.startsWith('en') ? 'en' : 'es',
        },
      })).unwrap();
      const fromPath = location.state?.from?.pathname || '/app';
      const fromSearch = location.state?.from?.search || '';
      navigate(fromPath + fromSearch, { replace: true });
    } catch (err) {
      if (err?.code !== 'APPLE_SIGN_IN_CANCELLED') {
        setLocalError(err?.message || t('social.appleError', 'No se pudo iniciar sesión con Apple'));
      }
    } finally {
      setAppleLoading(false);
    }
  };

  const visibleError = localError || (typeof error === 'string' ? error : error?.message);

  if (authChecked && isAuthenticated) {
    return null;
  }

  return (
    <AuthFormShell maxWidth="480px">
      <FormTitle>{t('login.login', 'Login')}</FormTitle>
      <FormSubtitle>{t('login.subtitle', 'Accede a tu cuenta para continuar')}</FormSubtitle>

      {(platform === 'ios' || !isNative) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <SocialButton
            type="button"
            onClick={handleAppleLogin}
            disabled={loading || appleLoading}
            style={{ background: '#000', borderColor: '#000', color: '#fff' }}
          >
            <FaApple aria-hidden="true" />
            {appleLoading ? '...' : t('social.apple', 'Continuar con Apple')}
          </SocialButton>

          {!isNative && <SocialButton type="button" onClick={handleGoogleLogin}>
            <svg width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            {t('login.continueWithGoogle', 'Continuar con Google')}
          </SocialButton>}
        </div>
      )}

      {(platform === 'ios' || !isNative) && <Divider>{t('common.or', 'o')}</Divider>}

      <Form onSubmit={onSubmit} noValidate>
        <Field>
          <InputLabel htmlFor="login-email">{t('login.email', 'Correo')}</InputLabel>
          <TextInput
            id="login-email"
            type="email"
            autoComplete="email"
            placeholder={t('login.email', 'Correo')}
            value={correo}
            onChange={(event) => setCorreo(event.target.value)}
          />
        </Field>

        <Field>
          <InputLabel htmlFor="login-password">{t('login.password', 'Contraseña')}</InputLabel>
          <TextInput
            id="login-password"
            type="password"
            autoComplete="current-password"
            placeholder={t('login.password', 'Contraseña')}
            value={password}
            onChange={(event) => setPassword(event.target.value)}
          />
        </Field>

        {visibleError && <ErrorMessage>{visibleError}</ErrorMessage>}

        <PrimaryButton type="submit" disabled={loading}>
          {loading ? '...' : t('login.buttonLogin', 'Iniciar sesión')}
        </PrimaryButton>
      </Form>

      <AccentLink to="/auth/forgot-password">{t('login.forgotPassword', 'Olvidé mi contraseña')}</AccentLink>
      {isNative ? (
        <>
          <div style={{ color: 'inherit', opacity: 0.68, fontSize: 13, textAlign: 'center' }}>
            {t('login.noAccount', '¿No tienes cuenta?')} {t('login.createAccountAtWebsite', 'Crea tu cuenta en')}{' '}
            <a
              href={websiteUrl(i18n.language, '/auth/register')}
              onClick={(event) => {
                event.preventDefault();
                openExternalWeb(websiteUrl(i18n.language, '/auth/register'));
              }}
              style={{ color: 'inherit', fontWeight: 700, textDecoration: 'underline' }}
            >
              xtramys.com
            </a>
          </div>
          <MutedAction type="button" onClick={() => openExternalWeb(websiteUrl(i18n.language, '/privacidad'))}>
            {t('legal.privacyPolicy', 'Política de privacidad')}
          </MutedAction>
          <MutedAction type="button" onClick={() => openExternalWeb(websiteUrl(i18n.language, '/eliminar-cuenta'))}>
            {t('legal.deleteAccount', 'Eliminar cuenta')}
          </MutedAction>
        </>
      ) : (
        <SecondaryLink to="/auth/register" state={{ from: location.state?.from }}>
          {t('login.createAccount', 'Crear cuenta')}
        </SecondaryLink>
      )}
    </AuthFormShell>
  );
}
