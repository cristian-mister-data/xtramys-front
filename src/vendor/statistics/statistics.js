...
export default function Statistics({ navigation: navigationProp }) {
    ...
    const matchSheetsKey = useMemo(() => {
        return matchSheets.map(m => `${m._id}-${m.golesFavor}-${m.golesContra}-${m.resultado}`).join(',');
    }, [matchSheets]);
...
playerStatsMap[p._id] = {
                ...
                injuries: injuries.filter(i => (typeof i.jugador === 'object' ? i.jugador._id : i.jugador) === p._id).length,
                totalTrainingSessions: pastTrainingSessions.length,
                trainingsAttended: trainingsAttended,
                attendancePercentage: pastTrainingSessions.length > 0
        ...
        playedMatches.forEach(match => {
            ...
            playedIds.forEach(pid => {
                if (playerStatsMap[pid] && !starters.includes(pid)) { // If played but not starter -> Sub In
                    playerStatsMap[pid].matches++;
                }
            });
        ...
        const getPositionPriority = (position) => {
            ...
            if (pos.includes('portero') || pos.includes('goalkeeper') || pos.includes('arquero')) return 1;
            if (pos.includes('defensa') || pos.includes('defender') || pos.includes('lateral') || pos.includes('central') || pos.includes('líbero') || pos.includes('libero') || pos.includes('zaguero') || pos.includes('carrilero')) return 2;
            if (pos.includes('centro') || pos.includes('medio') || pos.includes('midfielder') || pos.includes('pivote') || pos.includes('interior') || pos.includes('extremo') || pos.includes('volante')) return 3;
            if (pos.includes('delantero') || pos.includes('forward') || pos.includes('atacante') || pos.includes('punta') || pos.includes('ariete') || pos.includes('striker')) return 4;
        ...
        const rivalGoalStats = (() => {
            ...
            const avgFirstGoal = firstGoalMinutes.length > 0
            ...
            const avgPerMatch = playedMatches.length > 0
    ...
    }, [filteredMatchSheets, matchSheetsKey, players, injuries, trainingSessions, sortBy, sortOrder, competitionFilter]);
        ...
        return Object.values(weekMap)
            .map(week => ({
                ...
                playerAttendance: players
                    .filter(player => selectedPlayerIds.length === 0 || selectedPlayerIds.includes(player._id)) // Filter by selected players
                    .map(player => {
                        ...
                        return {
                            ...
                            percentage: week.sessions.length > 0
                                ? Math.round((attendedSessions.length / week.sessions.length) * 100)
                                : 0,
                            missedDates: missedSessions.map(s => new Date(s.fecha).toLocaleDateString(getLocale(), { weekday: 'short', day: 'numeric', month: 'short' })),
                        };
                    }),
            }))
    ...
    const generateInjuriesStatsHTML = () => {
        ...
        const zonesHTML = zoneList.length > 0
            ? zoneList.map(([name, count]) => {
        ...
        const typesHTML = typeList.length > 0
            ? typeList.map(([name, count]) => {
        ...
        if (activeInjuries.length > 0) {
            activeInjuriesRows = activeInjuries.map(inj => {
        ...
        <AppLayout>
            ...
            >
                        ...
                        <View style={styles.headerContent}>
                            <View style={styles.headerLeft}>
                                ...
                                <View style={styles.headerTextContainer}>
                                    <Text style={styles.seasonLabel}>{t('statistics.season')} {temporada?.nombre}</Text>
                                    <Text style={styles.teamName}>{selectedTeam?.nombre || t('statistics.yourTeam')}</Text>
                                </View>
                            </View>
                ...
                <View style={styles.pdfActionsContainer}>
                    <TouchableOpacity
                        style={styles.pdfPrimaryButton}
                        onPress={handleDownloadCombinedPDF}
                    >
                        <Ionicons name="document-text" size={18} color="#ffffff" style={{ marginRight: 8 }} />
                        <Text style={styles.pdfPrimaryButtonText}>{t('statistics.downloadCombined', 'Descargar Reporte Completo')}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        style={styles.pdfSecondaryButton}
                        onPress={handleDownloadActiveTabPDF}
                    >
                        <MaterialIcons name="picture-as-pdf" size={18} color={theme.colors.primary} style={{ marginRight: 8 }} />
                        <Text style={styles.pdfSecondaryButtonText}>
                            {activeTab === 'team' && t('statistics.downloadTeamPDF', 'Descargar PDF de Equipo')}
                            {activeTab === 'players' && t('statistics.downloadPlayersPDF', 'Descargar PDF de Jugadores')}
                            {activeTab === 'injuries' && t('statistics.downloadInjuriesPDF', 'Descargar PDF de Lesiones')}
                        </Text>
                    </TouchableOpacity>
                </View>
                        ...
                        return (
                            ...
                            >
                                ...
                                <Text style={[styles.tabText, activeTab === tab && styles.activeTabText]}>
                                    {tab === 'team' ? t('statistics.tabs.team') : tab === 'players' ? t('statistics.tabs.players') : t('statistics.tabs.injuries')}
                                </Text>
                            </TouchableOpacity>
                        );
                    })}
                ...
                {activeTab !== 'injuries' && (
                    <View style={styles.competitionFilterContainer}>
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.competitionTabs} contentContainerStyle={{ paddingHorizontal: 2 }}>
                            {[
                                ...
                                <TouchableOpacity
                                    key={tab.key}
                                    style={[styles.competitionTab, competitionFilter === tab.key && styles.competitionTabActive]}
                                    onPress={() => {
                                        setCompetitionFilter(tab.key);
                                    }}
                                >
                                    <Text style={[styles.competitionTabText, competitionFilter === tab.key && styles.competitionTabTextActive]} numberOfLines={1}>
                                        {tab.label}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                                ...
                                <View style={styles.rivalGoalCard}>
                                    <View style={styles.rivalGoalHeader}>
                                        ...
                                        <Text style={styles.rivalGoalMatchCount}>
                                            {stats.team.rivalGoalStats.matchesWithData} {t('statistics.team.matchesWithRivalGoalData')}
                                        </Text>
                                    </View>
                                    ...
                                    <View style={styles.rivalGoalHistogram}>
                                        {Object.entries(stats.team.rivalGoalStats.buckets).map(([period, count]) => {
                                            const maxBucket = Math.max(...Object.values(stats.team.rivalGoalStats.buckets), 1);
                                            const pct = (count / maxBucket) * 100;
                                            const isMostDangerous = period === stats.team.rivalGoalStats.mostDangerousPeriod;
                                            return (
                                                <View key={period} style={styles.rivalGoalBarRow}>
                                                    <Text style={styles.rivalGoalBarLabel}>
                                                        {period}
                                                    </Text>
                                    ...
                                    <View style={styles.goalsComparisonFooter}>
                                        <View style={styles.goalsComparisonFooterItem}>
                                            ...
                                            <Text style={styles.goalsComparisonFooterLabel}>{t('statistics.team.firstHalf')}</Text>
                                            <Text style={styles.goalsComparisonFooterValue}>{stats.team.rivalGoalStats.firstHalfGoals}</Text>
                                        </View>
                                        <View style={styles.goalsComparisonFooterDivider} />
                                        <View style={styles.goalsComparisonFooterItem}>
                                            <Text style={styles.rivalGoalHalfLabel}>2ª</Text>
                                            <Text style={styles.goalsComparisonFooterLabel}>{t('statistics.team.secondHalf')}</Text>
                                            <Text style={styles.goalsComparisonFooterValue}>{stats.team.rivalGoalStats.secondHalfGoals}</Text>
                                        </View>
                                        <View style={styles.goalsComparisonFooterDivider} />
                                        <View style={styles.goalsComparisonFooterItem}>
                            ...
                            <View style={styles.tacticalStatsGrid}>
                                <View style={styles.tacticalStatCard}>
                                    ...
                                    >
                                        ...
                                        <Text style={styles.tacticalStatLabel}>{t('statistics.preferredFormation')}</Text>
                                    </LinearGradient>
                                </View>
                                <View style={styles.tacticalStatCard}>
                                    <LinearGradient
                                        ...
                                        style={styles.tacticalStatGradient}
                                    >
                                        <Ionicons name="swap-horizontal-outline" size={isMobile ? 20 : 28} color="#ffffff" />
                        ...
                        <View style={styles.animateFadeIn}>
                            ...
                            <View style={styles.sortControlsContainer}>
                                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.sortControlsScroll}>
                                    {[
                                        { key: 'position', label: t('statistics.sortLabels.position'), icon: 'shirt-outline' },
                                        { key: 'minutes', label: t('statistics.sortLabels.minutes'), icon: 'time-outline' },
                                        { key: 'goals', label: t('statistics.sortLabels.goals'), icon: 'football-outline' },
                                        { key: 'assists', label: t('statistics.sortLabels.assists'), icon: 'hand-left-outline' },
                                        { key: 'matches', label: t('statistics.sortLabels.matchesPlayed'), icon: 'calendar-outline' },
                                        { key: 'attendancePercentage', label: t('statistics.sortLabels.attendance'), icon: 'checkmark-circle-outline' },
                                    ].map((item) => (
                                        <TouchableOpacity
                                            key={item.key}
                            ...
                            <View style={styles.positionLegendContainer}>
                                <View style={styles.positionLegendRow}>
                                    <View style={styles.positionLegendItem}>
                                        ...
                                        <Text style={styles.positionLegendText}>{t('statistics.positionLegend.goalkeeper')}</Text>
                                    </View>
                                    <View style={styles.positionLegendItem}>
                                        <View style={[styles.positionLegendDot, { backgroundColor: '#3b82f6' }]} />
                                        <Text style={styles.positionLegendText}>{t('statistics.positionLegend.defender')}</Text>
                                    </View>
                                    <View style={styles.positionLegendItem}>
                                        <View style={[styles.positionLegendDot, { backgroundColor: '#10b981' }]} />
                                        <Text style={styles.positionLegendText}>{t('statistics.positionLegend.midfielder')}</Text>
                                    </View>
                                    <View style={styles.positionLegendItem}>
                                        <View style={[styles.positionLegendDot, { backgroundColor: '#ef4444' }]} />
                                        <Text style={styles.positionLegendText}>{t('statistics.positionLegend.forward')}</Text>
                                    </View>
                                </View>
                            </View>
                                ...
                                <View style={styles.weeklyAttendanceSection}>
                                    <View style={styles.sectionCard}>
                                        ...
                                        <View style={styles.playerFilterContainer}>
                                            <Text style={styles.dateFilterLabel}>{t('statistics.weeklyAttendance.filterByPlayers')}:</Text>
                                            <TouchableOpacity
                                        ...
                                        <View style={styles.dateFilterContainer}>
                                            <View style={styles.dateFilterInputs}>
                                                <View style={styles.dateFilterItem}>
                                                    <Text style={styles.dateFilterLabel}>{t('statistics.weeklyAttendance.from')}:</Text>
                                                    <TouchableOpacity
                                                ...
                                                <View style={styles.dateFilterItem}>
                                                    <Text style={styles.dateFilterLabel}>{t('statistics.weeklyAttendance.to')}:</Text>
                                                    <TouchableOpacity
                                                        style={styles.dateFilterButton}
                                            ...
                                            <View style={styles.filterButtons}>
                                                <TouchableOpacity
                                                    style={[styles.applyFilterButton, !tempDateRange.start && !tempDateRange.end && styles.applyFilterButtonDisabled]}
                                                    onPress={() => {
                                                ...
                                                >
                                                    ...
                                                    <Text style={styles.applyFilterText}>{t('statistics.weeklyAttendance.applyFilter')}</Text>
                                                </TouchableOpacity>
                                                {(tempDateRange.start || tempDateRange.end || weeklyDateRange.start || weeklyDateRange.end) && (
                                                    <TouchableOpacity
                                                        ...
                                                        <View style={styles.weekHeader}>
                                                            <View style={styles.weekHeaderLeft}>
                                                                <Text style={styles.weekLabel}>
                                                                    {t('statistics.weeklyAttendance.weekOf')} {weekDate.toLocaleDateString(getLocale(), { day: 'numeric', month: 'short' })} - {weekEnd.toLocaleDateString(getLocale(), { day: 'numeric', month: 'short', year: 'numeric' })}
                                                                </Text>
                                                                <View style={styles.weekBadge}>
                                                                    <Text style={styles.weekBadgeText}>
                                                                        {week.totalSessions} {week.totalSessions === 1 ? t('statistics.weeklyAttendance.training') : t('statistics.weeklyAttendance.trainings')}
                                                                    </Text>
                                                                </View>
                                                            </View>
                                                            <TouchableOpacity
                                                                    ...
                                                                    <View key={playerAtt.playerId} style={styles.weekPlayerItem}>
                                                                        ...
                                                                        <View style={styles.weekPlayerStats}>
                                                                            ...
                                                                            <Text style={[
                                                                                ...
                                                                                {
                                                                                    color: playerAtt.percentage >= 80 ? '#10b981' :
                                                                                        playerAtt.percentage >= 60 ? '#f59e0b' : '#ef4444'
                                                                                }
                                                                            ]}>
                ...
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, { paddingBottom: Math.max(insets.bottom, 16) }]}>
                        ...
                        <View style={styles.modalActions}>
                            ...
                            >
                                <Text style={styles.modalActionText}>{t('statistics.weeklyAttendance.selectAll')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={styles.modalActionButton}
                                onPress={() => setSelectedPlayerIds([])}
                            >
                                <Text style={styles.modalActionText}>{t('statistics.weeklyAttendance.clearSelection')}</Text>
                            </TouchableOpacity>
                        </View>
                        ...
                        >
                            <Text style={styles.modalConfirmButtonText}>{t('statistics.weeklyAttendance.confirm')}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
