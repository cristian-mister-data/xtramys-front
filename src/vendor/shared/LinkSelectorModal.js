...
import { fetchExerciseFolders, fetchExerciseFoldersFlat } from '@/store/slices/exercise/exerciseThunks';
import { fetchStrategyFolders, fetchStrategyFoldersFlat } from '@/store/slices/strategy/strategyThunks';

/* ─────────────── Theme ─────────────── */
const THEME_DEFAULT = {
  primary: '#2474E5', bg: '#f8fafc', surface: '#ffffff',
  text: '#1e293b', textSec: '#64748b', textMuted: '#94a3b8',
  border: '#e2e8f0', surfaceAlt: '#f1f5f9',
  primarySoft: '#eff6ff', primarySoftBorder: '#bfdbfe',
};
const buildTheme = (sc) => {
  const c = sc?.colors || {};
  return {
...
export default function LinkSelectorModal({ type, visible, onClose, onSelect }) {
  ...
  }, [reduxFolders, reduxFoldersFlat, currentFolderId, getFolderName]);
  ...
  const displayItems = useMemo(() => {
    ...
    if (!lower) return source.slice().sort((a, b) => (a.nombre || '').localeCompare(b.nombre || ''));
    return source
        ...
        <View style={[s.container, isWide && { alignSelf: 'center', maxWidth: 700 }]}>
          ...
          <View style={s.header}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              <View>
                <Text style={s.title}>
                  {isExercise ? t('myVideos.linkToExerciseTitle') : t('myVideos.linkToStrategyTitle')}
                </Text>
              ...
              {folderPath.map((c, i) => (
                <View key={c._id} style={s.crumbItem}>
                  ...
                  >
                    ...
                    <Text style={[s.crumbText, i === folderPath.length - 1 && s.crumbActive]} numberOfLines={1}>
                      {c.nombre}
                    </Text>
                  </TouchableOpacity>
                </View>
          ...
          {loading ? (
            ...
            <ScrollView style={s.list} contentContainerStyle={{ paddingBottom: 16 }} showsVerticalScrollIndicator>
                  ...
                  {isWide ? (
                    ...
                    <View style={s.foldersGrid}>
                      {currentLevelFolders.map(folder => (
                        <TouchableOpacity
                          ...
                          activeOpacity={0.7}
                        >
                          <View style={[s.folderCardIcon, { backgroundColor: (folder.color || THEME.primary) + '1A' }]}>
                            <Ionicons name="folder" size={22} color={folder.color || THEME.primary} />
                          </View>
                          <View style={{ flex: 1 }}>
                    ...
                    currentLevelFolders.map(folder => (
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                      >
                        <View style={[s.folderRowIcon, { backgroundColor: (folder.color || THEME.primary) + '1A' }]}>
                          <Ionicons name="folder" size={20} color={folder.color || THEME.primary} />
                        </View>
                        <Text style={s.folderRowName} numberOfLines={1}>{getFolderName(folder)}</Text>
                  ...
                  <Text style={s.emptyTxt}>
                    {isSearching
                      ...
                      : (currentFolderId
                        ...
                        : (isExercise ? t('myVideos.noExercises', 'Sin ejercicios') : t('myVideos.noStrategies', 'Sin estrategias'))
                      )}
                  </Text>
                </View>
              ) : (
                displayItems.map(item => {
                  const folderName = isSearching && item.folder && typeof item.folder === 'object'
                    ? item.folder.nombre : null;
                  return (
                    <TouchableOpacity
                      key={item._id}
                      style={s.itemRow}
                      ...
                      activeOpacity={0.75}
                    >
                      <View style={[s.itemIcon, { backgroundColor: isExercise ? '#E0F2FE' : '#FDF4FF' }]}>
                        <Ionicons name={iconName} size={18} color={accentColor} />
                      </View>
                      <View style={{ flex: 1 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                          <Text style={s.itemName} numberOfLines={2}>{item.nombre}</Text>
                          {item.isGlobal && (
                        ...
                        {folderName && (
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 2 }}>
                            <Ionicons name="folder" size={10} color={item.folder?.color || THEME.textSec} />
